...
if (isset($_GET['del'])) {
   ...
   $id = $_GET['id'];
	...
	mysqli_query($conn, "DELETE FROM `location` WHERE loc_id = $id");
}

!isset($_SESSION['login']) ? header("Location: index.php") : '' ;


$items = query("SELECT * FROM `location`");

if (isset($_POST['search'])) {
   $key = $_POST['keyword'];

   $items = query("SELECT * FROM `location` WHERE
				loc_name LIKE '%$key%'");
}
	...
	<body>
      ...
      <?php if($_SESSION['role'] == 'ADMIN'): ?>
         ...
         <div class="d-flex mt-2 mx-5">
            <a href="addlocation.php" class="btn btn-success text-white py-3 " style="text-decoration: none; margin-bottom:0; flex:4;"><i class="fas fa-plus"></i> Add location</a>
         </div>
         <?php endif; ?>
            ...
            <tbody>
                  ...
                  <tr>
                     <th scope="row"><?= $item['loc_id']; ?></th>
                     <td><?= $item['loc_name']; ?></td>
                     <td class="d-flex flex-column ">
                        <a href="locEdit.php?id=<?= $item['loc_id']; ?>" style="text-decoration: none;" class="text-white btn btn-primary">edit</a>
                        <a href="location.php?id=<?= $item['loc_id']; ?>&del=" style="text-decoration: none;" class="text-white btn btn-danger">delete</a>
                     </td>
                  </tr>
                     <?php endforeach; ?>
            </tbody>
